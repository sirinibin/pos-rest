...
// PostingPrintData returns the stored posting print job for chromedp's React page.
// No authentication required — the key itself is an unguessable random secret.
// GET /v1/posting/print-data/{key}
func PostingPrintData(w http.ResponseWriter, r *http.Request) {
	key := mux.Vars(r)["key"]
...
// PostingPDF accepts fully-loaded balance sheet / posting journal data from React,
// stores it under a random key, then uses headless Chrome to render the React
// /posting-print page (which reads the data via that key) and captures an A4 PDF.
//
// POST /v1/posting/pdf
// Body: { "model": {...}, "modelName": "balance_sheet", "fontSizes": {...}, "filename": "..." }
func PostingPDF(w http.ResponseWriter, r *http.Request) {
	w.Header().Set("Content-Type", "application/json")
	...
	if saveFilename == "" {
		saveFilename = fmt.Sprintf("posting_%s_%d", reqBody.ModelName, time.Now().Unix())
	}
