...
func (model *Purchase) MakeRedisCode() error {
	store, err := FindStoreByID(model.StoreID, bson.M{})
	if err != nil {
		return err
	}

	redisKey := model.StoreID.Hex() + "_purchase_invoice_counter"
	...
	if exists == 0 {
		count, err := store.GetPurchaseCount()
		if err != nil {
			return err
		}
	...
	if store.PurchaseSerialNumber.Prefix != "" {
		model.Code = fmt.Sprintf("%s-%0*d", store.PurchaseSerialNumber.Prefix, paddingCount, incr)
	} else {
		model.Code = fmt.Sprintf("%s%0*d", store.PurchaseSerialNumber.Prefix, paddingCount, incr)
	}
