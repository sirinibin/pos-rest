...
type ProductNonVATSalesHistory struct {
...
type NonVATSalesHistoryStats struct {
	TotalNonVATSales float64 `json:"total_non_vat_sales" bson:"total_non_vat_sales"`
...
func (store *Store) GetNonVATSalesHistoryStats(filter map[string]interface{}) (stats NonVATSalesHistoryStats, err error) {
	collection := db.GetDB("store_" + store.ID.Hex()).Collection("product_non_vat_sales_history")
	...
	pipeline := []bson.M{
		...
		{"$group": bson.M{
			"_id":                nil,
			"total_non_vat_sales": bson.M{"$sum": "$net_price"},
	...
	if cur.Next(ctx) {
		...
		stats.TotalNonVATSales = RoundFloat(stats.TotalNonVATSales, 2)
...
func (store *Store) SearchNonVATSalesHistory(w http.ResponseWriter, r *http.Request) (models []ProductNonVATSalesHistory, criterias SearchCriterias, err error) {
	...
	if err != nil {
		return models, criterias, errors.New("Error fetching non vat sales history: " + err.Error())
	}
	if cur != nil {
		defer cur.Close(ctx)
	}
	...
	for i := 0; cur != nil && cur.Next(ctx); i++ {
		...
		model := ProductNonVATSalesHistory{}
...
func (nonVATSales *NonVATSales) CreateProductsNonVATSalesHistory() error {
	store, err := FindStoreByID(nonVATSales.StoreID, bson.M{})
	if err != nil {
		return err
	}

	exists, err := store.IsNonVATSalesHistoryExistsByNonVATSalesID(&nonVATSales.ID)
	if err != nil {
		return err
	}
	if exists {
	...
	collection := db.GetDB("store_" + nonVATSales.StoreID.Hex()).Collection("product_non_vat_sales_history")
	ctx, cancel := context.WithTimeout(context.Background(), 5*time.Second)
	defer cancel()

	vatPercent := float64(0)
	if nonVATSales.VatPercent != nil {
		vatPercent = *nonVATSales.VatPercent
	}

	for _, p := range nonVATSales.Products {
		history := ProductNonVATSalesHistory{
			ID:                 primitive.NewObjectID(),
			Date:               nonVATSales.Date,
			StoreID:            nonVATSales.StoreID,
			StoreName:          nonVATSales.StoreName,
			ProductID:          p.ProductID,
			CustomerID:         nonVATSales.CustomerID,
			CustomerName:       nonVATSales.CustomerName,
			CustomerNameArabic: nonVATSales.CustomerNameArabic,
			NonVATSalesID:      &nonVATSales.ID,
			NonVATSalesCode:    nonVATSales.Code,
			Quantity:           p.Quantity,
			Unit:               p.Unit,
			Discount:           p.UnitDiscount,
			DiscountPercent:    p.UnitDiscountPercent,
			CreatedAt:          nonVATSales.CreatedAt,
			UpdatedAt:          nonVATSales.UpdatedAt,
		}
		...
		if len(product.Set.Products) > 0 {
			for _, setProduct := range product.Set.Products {
				...
				setHistory := ProductNonVATSalesHistory{
					ID:                 primitive.NewObjectID(),
					Date:               nonVATSales.Date,
					StoreID:            nonVATSales.StoreID,
					StoreName:          nonVATSales.StoreName,
...
func (store *Store) IsNonVATSalesHistoryExistsByNonVATSalesID(ID *primitive.ObjectID) (exists bool, err error) {
	collection := db.GetDB("store_" + store.ID.Hex()).Collection("product_non_vat_sales_history")
	ctx, cancel := context.WithTimeout(context.Background(), 5*time.Second)
	defer cancel()
	count, err := collection.CountDocuments(ctx, bson.M{"non_vat_sales_id": ID})
	return (count > 0), err
}

func (nonVATSales *NonVATSales) ClearProductsNonVATSalesHistory() error {
	collection := db.GetDB("store_" + nonVATSales.StoreID.Hex()).Collection("product_non_vat_sales_history")
	ctx := context.Background()
	_, err := collection.DeleteMany(ctx, bson.M{"non_vat_sales_id": nonVATSales.ID})
	return err
}

func (model *ProductNonVATSalesHistory) Update() error {
	collection := db.GetDB("store_" + model.StoreID.Hex()).Collection("product_non_vat_sales_history")
	ctx, cancel := context.WithTimeout(context.Background(), 5*time.Second)
	defer cancel()
